...
public class ClockDetail extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title.setText("设置闹钟");
        calendar = Calendar.getInstance(); //获得一个Calendar类型的通用对象，getInstance()将返回一个Calendar的对象
        initView();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.set_time:
                calendar.setTimeInMillis(System.currentTimeMillis());  //取得系统时间
               ...
               //intent.putExtra("content",clock.getContent());
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    if (System.currentTimeMillis()>calendar.getTimeInMillis()+60000){
                        //加24小时                        am.setExact(AlarmManager.RTC_WAKEUP, calendar.getTimeInMillis()+86400000, sender);
                    }else {
    ...
    private String format(int x) { //把int 的1置为01
        String s = "" + x;
        if (s.length() == 1) {
            s = "0" + s;
        }
    ...
    private String formatString(String x) {//把string 的1置为01
        String s = x;
        if (s.length() == 1) {
            s = "0" + s;
        }
